...
Railsバックエンドに関しては、過去にもリファクタリングに挑んだことがあり、そのときどきのレイヤーが残って地層となっている状態だった。コードリーディングに際して、残っているドキュメントと現在の構造を頭の中で整合させながら読むという、新たにジョインしたエンジニアにとっては非常に難読な課題が一つ（私もその一人）。

また新たに機能追加するにおいても、どの地層に合わせた構造にするべきなのかが決まっておらず、最終的に個々の判断に委ねられるため、さらに統一性が失われるという課題もあった。

ユーザーの多様なニーズに対応するための機能追加
...
### DDD（ドメイン駆動設計）
システムを構成するビジネスドメイン（領域）を中心にソフトウェア設計を行うアプローチ。
...
所感だが、この3つの個別のアプローチを組み合わせなくとも、DDDだけで技術的負債を解消していく試みはおそらく不可能ではない。ただ自社もそうだが同様の課題を抱えているように見えてコンテキストが大きく異なるのは確かだと思われるので、One size fit allの解決策はなく、より広範なフレームワークがあることは有用である。（※もちろｎフレームワークに功罪あることはご承知かと思う）
